...
namespace linq
{
    ...
    class Program
    {
        static void Main()
        {
            ...
            //Console.WriteLine($"Most expensive: {prices.Max()}");
            ...
            */
            ...
            List<int> notSquareYet = wheresSquaredo.TakeWhile(n => Math.Sqrt(n) - Math.Floor(Math.Sqrt(n)) != 0 ).ToList();

            foreach (int num in notSquareYet) {
                //Console.WriteLine($"The root of {num} is {Math.Sqrt(num)}");
            }
            ...
            foreach (Customer customer in millionaires) {
                Console.WriteLine($"{customer.Name} is a millionaire.");
            }
